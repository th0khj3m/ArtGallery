...
export class PaymentCardPipe implements PipeTransform {
  ...
  transform(value?: ConfirmationToken["payment_method_preview"] | PaymentSummary, ...args: unknown[]): unknown {
    if (value && 'card' in value) {
      ...
      return `${brand.toUpperCase()} ${Array(3).fill("*".repeat(4)).join(" ")} ${last4}, Exp: ${exp_month}/${exp_year}`
    }
    else if (value && 'last4' in value) {
      const { brand, last4, expMonth, expYear } =
        (value as PaymentSummary);
      return `${brand.toUpperCase()} ${Array(3).fill("*".repeat(4)).join(" ")} ${last4}, Exp: ${expMonth}/${expYear}`
    }
    else {
